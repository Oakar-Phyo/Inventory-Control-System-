...
public class ReceiveReportPDFExporter {
	private List<StockIn> listStock;
	
	 public  ReceiveReportPDFExporter(List<StockIn> listStock) {
	 ...
	 private void writeTableHeader(PdfPTable table) {
	        ...
	        cell.setPhrase(new Phrase("Receive No", font));
	        table.addCell(cell);
	 ...
	 private void writeTableData(PdfPTable table) {
			int totalNo = 1;
	        for (StockIn receivereturn : listStock) {
	        	table.addCell(String.valueOf(totalNo));
	        	table.addCell(receivereturn.getCode());
	            table.addCell(receivereturn.getStock().getStockCode());
	            table.addCell(receivereturn.getStock().getName());
	            table.addCell(receivereturn.getWarehouse().getName());
	            table.addCell(receivereturn.getSupplier().getName());
	            table.addCell(String.valueOf(receivereturn.getStock().getUnit().getName()));
	            table.addCell(String.valueOf(receivereturn.getQty()));
	            table.addCell(String.valueOf(receivereturn.getDate()));
...
//	        pdfPTable.addCell(pdfPCell10);
	        ...
	        //Adding Header
	        Paragraph p = new Paragraph("Inventory Receive Report", font);
	       ...
	       //Upper Part of the Page Ends(Hidden Table)
	        ...
	        table.setWidths(new float[] {1.5f, 3.5f, 3.5f, 4.0f, 4.5f, 4.5f,3.0f,3.5f,5.0f,3.0f,3.5f});

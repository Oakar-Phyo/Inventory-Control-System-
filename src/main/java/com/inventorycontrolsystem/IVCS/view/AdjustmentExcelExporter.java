...
public class AdjustmentExcelExporter {

	//Export Inventory Receive
		private XSSFWorkbook workbook;
		private XSSFSheet sheet;
		private List<Adjustment> adjusmentList;

		public AdjustmentExcelExporter(List<Adjustment> adjusmentList) {
			this.adjusmentList = adjusmentList;
		...
		private void writeHeaderLine() {
			...
			createCell(row, 4, "Adjustment Quantity", style);
		...
		private void writeDataLines() {
			...
			// Top Data Rows
			createCell(sheet.createRow(0), 0, "Inventory Adjustment Transaction Details" , styleH);
			
			//Upper Part(Table is Created)
			//This data needs to be written (Object[])
	        Map<String, Object[]> data = new TreeMap<String, Object[]>();

			data.put("1", new Object[] { "Adjustment No.:  " + adjusmentList.get(0).getAdjustmentCode(),
			"Date" + adjusmentList.get(0).getDate() });
			data.put("2", new Object[] { "Warehouse Code:  " + adjusmentList.get(0).getWarehouse().getWarehouseCode()});
			data.put("3", new Object[] { "Warehouse Name:  " + adjusmentList.get(0).getWarehouse().getName()});
			...
			int tQty = 0;
			double tAmt = 0;
			items = adjusmentList.size();

			// Table Rows(Table Header is Created by WriteHeaderLine())
			for (Adjustment adjustment : adjusmentList) {
				...
				createCell(row, columnCount++, adjustment.getStock().getUnit().getName(), style);
			...
			// Total & Remark
			createCell(sheet.createRow(16), 0, "Remark:  " + adjusmentList.get(0).getRemark() , style1);

...
public class AdjustmentPDFExporter {
...
private List<Adjustment> adjustmentList;
    ...
    public AdjustmentPDFExporter(List<Adjustment> adjustmentList) {
    ...
    private void writeTableData(PdfPTable table) {
    	items=adjustmentList.size();
        for (Adjustment adjustment : adjustmentList) {
            table.addCell(String.valueOf(adjustment.getStock().getStockCode()));
            table.addCell(adjustment.getStock().getName());
            table.addCell(adjustment.getStock().getUnit().getName());
            ...
            tQty =tQty+adjustment.getTotalAdjustment();
			  tAmt = tAmt+adjustment.getSubTotalAmount();
        }
    }
    ...
    public void export(HttpServletResponse response) throws DocumentException, IOException {
      ...
      //Create Table object, Here 2 specify the no. of columns
        ...
        //Create cells
        PdfPCell pdfPCellNo = new PdfPCell(new Paragraph("Adjustment No.:  " + adjustmentList.get(0).getAdjustmentCode()));
        PdfPCell pdfPCellWCode = new PdfPCell(new Paragraph("Warehouse Code:  " + adjustmentList.get(0).getWarehouse().getWarehouseCode()));
        PdfPCell pdfPCellWName = new PdfPCell(new Paragraph("Warehouse Name:  " +adjustmentList.get(0).getWarehouse().getName()));
        PdfPCell pdfPCellSDate = new PdfPCell(new Paragraph("Adjustment Date:  " +adjustmentList.get(0).getDate()));
		 ...
		 */
        //Adding Header
        Paragraph p = new Paragraph("Inventory Adjustment Transaction", font);
       ...
       //Upper Part of the Page Ends(Hidden Table)
        ...
        Paragraph p1 = new Paragraph("Remark: " +adjustmentList.get(0).getRemark());
        p1.setAlignment(Paragraph.ALIGN_LEFT);
        document.add(p1);
        Paragraph p2 = new Paragraph("Items: " + adjustmentList.size());
